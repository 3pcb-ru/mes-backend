...
export class DrizzleHealthIndicator {
    ...
    async pingCheck(key: string, options: { timeout?: number } = {}): Promise<HealthIndicatorResult> {
        ...
        try {
            ...
            await this.executeWithTimeout(this.drizzleService.database.execute('SELECT 1'), timeout);
    ...
    private async executeWithTimeout<T>(promise: Promise<T>, timeoutMs: number): Promise<T> {
        return Promise.race([promise, new Promise<T>((_, reject) => setTimeout(() => reject(new Error(`Database query timeout after ${timeoutMs}ms`)), timeoutMs))]);
    }
}
