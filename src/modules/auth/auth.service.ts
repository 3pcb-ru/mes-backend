...
export class AuthService {
    ...
    async signup(signupDto: SignupDto) {
        ...
        try {
            ...
            if (error instanceof BadRequestException || error instanceof PreconditionFailedException || error instanceof ConflictException) {
                throw error;
            }
    ...
    async login(loginDto: LoginDto) {
        ...
        if (!user.isVerified) {
            ...
            // User is allowed to login, but we'll return the isVerified: false flag
            this.logger.log('Unverified user logged in', { email: user.email });
        }
